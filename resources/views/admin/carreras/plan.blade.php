...
@section('contenido')
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-400">{{ $carrera->nombre_carrera }} — Oferta académica vigente e itinerarios institucionales</p>
    </div>
        ...
        <table class="w-full text-xs border-collapse">
            <thead>
                <tr class="bg-slate-50 text-slate-600">
                    ...
                    <th class="border border-slate-200 px-2 py-2">Período</th>
                ...
                @foreach ($materiasPorAnio as $anio => $materias)
                    @foreach ($materias as $i => $materia)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="border border-slate-200 text-center text-slate-500">{{ $materia->periodo->nombre_periodo }}</td>
